...
class AfipwsConnection(models.Model):
    ...
    def get_afip_ws_url(self, afip_ws, environment_type):
        ...
        """
        _logger.debug('Getting URL for afip ws %s on %s' % (
    ...
    def connect(self):
        ...
        # cache = cfg.get_param(cr, uid, 'py3afipws.cache', context=context)
        # proxy = cfg.get_param(cr, uid, 'py3afipws.proxy', context=context)

        wsdl = self.afip_ws_url
        _logger.debug("### afipws_connection ### wsdl :" + str(wsdl))
    ...
    def _get_ws(self, afip_ws):
        ...
        """
        _logger.debug('Getting ws %s from libraries ' % afip_ws)
        # por ahora el unico implementado es ws_sr_padron_a4
        ws = False
        if afip_ws == 'ws_sr_padron_a4':
            from py3afipws.ws_sr_padron import WSSrPadronA4
            ws = WSSrPadronA4()
        return ws
