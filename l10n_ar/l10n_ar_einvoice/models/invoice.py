...
try:
    ...
    _logger.debug('Can not `from py3afipws.soap import SoapFault`.')
...
print "Observaciones:", wscdc.Obs
        ...
        for inv in self:
            ...
            _logger.debug('Constatando Comprobante en afip')
    ...
    def do_pyafipws_request_cae(self):
        ...
        _logger.debug('-----------do_pyafipws_request_cae--------------------')
        for inv in self:
            ...
            last_invoice = inv.journal_document_type_id\
                .get_pyafipws_last_invoice()
            _logger.debug('Last Invoice: %s' % str(last_invoice))
            ws_next_invoice_number = int(last_invoice['result']) + 1
            ...
            try:
                ...
                _logger.debug('except SoapFault')
                msg = 'Falla SOAP %s: %s' % (
                    fault.faultcode, fault.faultstring)
            except Exception as e:
                _logger.debug('except Exception as e')
                msg = str(e)
                _logger.debug('msg: %s' % msg)
            except Exception:
                _logger.debug('except Exception')
                if ws.Excepcion:
                    # get the exception already parsed by the helper
                    msg = ws.Excepcion
                    _logger.debug('msg ws.Excepcion: %s' % str(msg))
                else:
                    # avoid encoding problem when raising error
                    _logger.debug('else')
                    msg = traceback.format_exception_only(
                    ...
                    _logger.debug('msg: %s' % str(msg))
            if msg:
                _logger.debug('if msg')
                _logger.debug('msg %s' % str(msg))
            ...
            _logger.debug('CAE solicitado con exito. CAE: %s. Resultado %s' % (
                ws.CAE, ws.Resultado))
            inv.write({
                'afip_auth_mode': 'CAE',
