...
class ResPartner(models.Model):
    ...
    def onchange_country_id(self):
        self.afip_responsability_type_id = False
        self.main_id_number = False
        self.main_id_category_id = False
    ...
    def get_data_from_padron_afip(self):
        ...
        _logger.debug("____Calling ws_sr_padron_a4_____")
        padron = company.get_connection('ws_sr_padron_a4').connect()
        _logger.debug("____Called ws_sr_padron_a4_____")
        _logger.debug("__padron___:" + str(padron))
    ...
    def create(self, vals):
        PartnerCountry = self.env['res.country'].browse(
            vals.get('country_id', False))
        afip_responsability_type_id = vals.get(
            'afip_responsability_type_id', False)

        if PartnerCountry and PartnerCountry.id != self.env.ref('base.ar').id \
           and not afip_responsability_type_id:
            if vals.get('supplier', False):
                vals.update({
                    'afip_responsability_type_id': self.env.ref(
                        'l10n_ar_account.res_EXT').id
                })
            else:
                vals.update({
                    'afip_responsability_type_id': self.env.ref(
                        'l10n_ar_account.res_CLI_EXT').id
                })
                if vals.get('company_type', '') == 'person':
                    vals.update({
                        'main_id_number': PartnerCountry.cuit_fisica,
                        'main_id_category_id': self.env.ref(
                            'l10n_ar_base.dt_CUIT').id
                    })
                else:
                    vals.update({
                        'main_id_number': PartnerCountry.cuit_juridica,
                        'main_id_category_id': self.env.ref(
                            'l10n_ar_base.dt_CUIT').id
                    })
    ...
    def write(self, vals):
        ...
        PartnerCountry = self.env['res.country'].browse(country_id)
        afip_responsability_type_id = vals.get(
            'afip_responsability_type_id', self.afip_responsability_type_id.id)
        supplier = vals.get('supplier', self.supplier)
        ...
        if PartnerCountry and PartnerCountry.id != self.env.ref('base.ar').id \
           and not afip_responsability_type_id:
            if supplier:
                ...
                if vals.get('company_type', self.company_type) == 'person':
                    vals.update({
                        'main_id_number': PartnerCountry.cuit_fisica,
                        'main_id_category_id': self.env.ref(
                            'l10n_ar_base.dt_CUIT').id
